---
name: llm-wiki-architecture
description: "Use when asked to sync, update, generate, maintain, review, or critique architecture docs in .arch; when requested to perform architecture weakness/risk/NFR/deployability analysis; or when major code changes may require architecture refresh and drift/risk updates."
---

# LLM-Wiki Architecture Skill
...
7. Apply mandatory visibility cues for scanability in every architecture update/refresh/sync:
   ...
   - In each Scan First section, include exactly 3 bullets in this order: red Act now, yellow Watch closely, green Stable base.
...
- [ ] Deployability assessment in .arch/system-overview.md has been re-evaluated against current infrastructure/CI/CD state
...
- [ ] Confirm that supply chain vulnerability checks (for example dependency audits) have been run and are clean, or document exceptions in .arch/risks.md
...
- [ ] .arch/README.md, .arch/system-overview.md, .arch/risks.md, and .arch/drift.md include ## Scan First (Traffic Light) sections with red/yellow/green bullets in required order
...
For each architecture sync or review, include a concise NFR scorecard in relevant docs (typically .arch/system-overview.md, .arch/risks.md, and .arch/drift.md):
...
- Current: embedded recommendation lists are checked for validity and updated when assumptions change.
...
- .arch/components.md should stay concise and high-level: list components, summarize their relationships, and link to per-component detail files.
...
- Prefer active voice and present tense in all arch content.
- For risk and weakness findings, use a consistent mini-structure rather than dense prose paragraphs. Each finding should have: a short bold title, then What, Why it matters, Evidence, Resolution on separate lines.
...
- Threat model sections must use a two-part structure: first a plain-English summary paragraph (one or two sentences stating overall exposure and top concern), then a table with columns: Trust Boundary | What crosses it | Current gap | Severity.
- For each threat boundary gap, state customer impact in plain language before the technical description.
